...
async function fetchCourseCoords(courseId: string, apiKey: string): Promise<MetrixCourse | null> {
    try {
...
export const competitionsQueries = {
    Query: {
        getWeekliesNearMe: async (
            ...
            if (await weekliesRateLimiter({ parent, args, context, info }, { max: 10, window: "1m" })) {
            ...
            const res = await fetch(url);
            const data = await res.json() as { Competitions?: MetrixCompetition[] };
            ...
            const withCoords = await Promise.all(
                competitions.map(async (c) => {
                    ...
                    const distance = getDistance(
                        ...
                        { latitude: parseFloat(course.Lat), longitude: parseFloat(course.Lng) }
                    );
